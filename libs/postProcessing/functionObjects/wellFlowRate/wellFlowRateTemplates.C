...
template<class Type>
void Foam::wellFlowRate::calcMinMaxFields(const word& fieldName)
{
    ...
    if (obr_.foundObject<fieldType>(fieldName))
    {
        ...
        switch (mode_)
        {
            case mdMag:
            {
                ...
                if (Pstream::master())
                {
                    wellFlowRateFilePtr_() << obr_.time().value() << tab
                        << fieldName << tab << minValue << tab << maxValue
                        << endl;

                    if (log_)
                    {
                        Info<< "wellFlowRate output:" << nl
                            << "    min(mag(" << fieldName << ")) = "
                            << minValue << nl
                            << "    max(mag(" << fieldName << ")) = "
                            << maxValue << nl
            ...
            default:
            {
                FatalErrorIn
                (
                    "Foam::wellFlowRate::calcMinMaxFields"
