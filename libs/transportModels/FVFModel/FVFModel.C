...
(
    ...
    (
        ...
        p_.mesh(),
        dimensionedScalar("rFVF", dimless, 0.0),
        "zeroGradient"
    ),
    drFVFdP_
    (
...
)
{
    ...
    {
        FatalErrorIn("Foam::autoPtr<Foam::FVFModel> Foam::FVFModel::New\
                    (\
                        const word& name,\
                        const dictionary& phaseDict,\
                        const volScalarField& p\
    ...
    return autoPtr<FVFModel>
        (cstrIter()(name, phaseDict, p));
}
// ************************************************************************* //
