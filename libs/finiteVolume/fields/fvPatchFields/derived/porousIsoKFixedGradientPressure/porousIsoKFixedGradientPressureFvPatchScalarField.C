...
#include "porousIsoKFixedGradientPressureFvPatchScalarField.H"
...
Foam::porousIsoKFixedGradientPressureFvPatchScalarField
    ::porousIsoKFixedGradientPressureFvPatchScalarField
(
    const porousIsoKFixedGradientPressureFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::porousIsoKFixedGradientPressureFvPatchScalarField
         ::updateCoeffs()
{
    if (updated())
    {
    ...
    // I don't think fondly of activators but may become handy at some point
    //scalar  activateCapillarity(
    //db().lookupObject<dictionary>("transportProperties")
    //.
    //lookupOrDefault<scalar>("activateCapillarity",0.));
