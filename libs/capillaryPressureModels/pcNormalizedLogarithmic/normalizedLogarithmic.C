...
#include "normalizedLogarithmic.H"
...
namespace Foam
{
  ...
  {
    defineTypeNameAndDebug(normalizedLogarithmic, 0);
...
Foam::capillaryPressureModels::normalizedLogarithmic::normalizedLogarithmic
(
    const word& name,
...
Se_((Sw_- pcSmin_)/(pcSmax_-pcSmin_))
{
    ...
    {
        FatalErrorIn("Foam::capillaryPressureModels::normalizedLogarithmic::normalizedLogarithmic\
                    (\
                        const word& name,\
                        const dictionary& transportProperties,\
                        const phase& wettingPhase,\
...
void Foam::capillaryPressureModels::normalizedLogarithmic::correct()
{
    Se_ == (Sw_-pcSmin_)/(pcSmax_ - pcSmin_);
    pc_ = -pc0_ * log10(Se_);
    dpcdS_ = -pc0_/(pcSmax_ - pcSmin_)/(Se_+VSMALL)/log(10.0);
